...
document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(([idols, parsedData]) => {
        ...
        populateFilters();
        setupGroupSearchAndFilters();
        renderGroupTab();
    })
    .catch(err => {
        console.error("Error loading groups dashboard:", err);
    });
});
...
function populateFilters() {
    ...
    if (groupTabAgencySelect) {
        ...
        agencies.forEach(agency => {
            groupTabAgencySelect.innerHTML += `<option value="${agency}">${agency}</option>`;
        });
    }
}

function setupGroupSearchAndFilters() {
